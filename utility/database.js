...
var options = {
	connectionLimit : 100, // important - limit the number of simultaneous
	// connection
...
pool.on('error', function(err) {
	debug("MySQL Error: " + err.errno + " " + err.code + " "
			+ (err.fatal ? "{FATAL}" : "{NOT FATAL}"));
	// debug(err);
});
 ...
 * @param callback
 *            (err, result) : if no error then error is null, otherwise result
 *            is null
 */
 ...
 * 
 * @param callback
 *            (err, res)=> res = list of all vaccine object.
 */
...
var createUser = function(uname, email, password, name, callback) {
	...
	runQuery(sql, function(err, res) {
		if (res && res.length > 0) {
			...
			inserts = [ 'user', 'uname', 'email', 'password', 'name', uname,
					email, password, name ];
			sql = mysql.format(sql, inserts);
 ...
 * @param callback
 *            (err, res) where res = true on success.
 */
 ...
 *            New password
 * @param callback
 *            (err, result) => err is null if success
 */
 ...
 *            ID of the user
 * @param callback
 *            (err, res) => array of child objects
 */
var getChildrenOf = function(id, callback) {
	var sql, selects;
	sql = "SELECT * FROM ?? ,?? WHERE child.id=guardian.childfk and ?? = ?";
	selects = [ 'child','guardian','user', id ];
	sql = mysql.format(sql, selects);
	console.log("get children sql");
	console.log(sql);
	runQuery(sql, callback);
};
 ...
 * 
 * @param user
 *            ID of user this child belongs to
 * @param name
 *            Name of the child
 * @param dob
 *            Date of birth
 ...
 *            Gender of the child
 * @param callback
 *            (err, res) => res = newly created child object
 */
var createChild = function(user, name, dob, gender, callback) {
	getChildByName(user, name, function(err, data) {
		if (data && data.length > 0) {
			...
			var sql, inserts;
			sql = "INSERT INTO ?? (??,??,??,??) VALUES (?,?,?,?)";
			inserts = [ 'child', 'user', 'name', 'dob', 'gender', user, name,
					dob, gender ];
			sql = mysql.format(sql, inserts);
 ...
 *            (err, res) => res = newly created child object
 */
var createChildWithParent = function(user, name, dob, gender, parent_name,
		parent_mobile_number, parent_email_id, callback) {
	getChildByName(user, name, function(err, data) {
		if (data && data.length > 0) {
			callback("This child has already been added");
		} else {
			...
			runQuery(sql, function(err, result) {
				if (err) {
					...
					sql = "INSERT INTO ?? (??,??,??,??) VALUES (?,?,?,?)";
					inserts = [ 'guardian', 'childfk', 'mobile_number',
							'email', 'parent_name' ,result.insertId, parent_mobile_number,
							parent_email_id ,parent_name];
					console.log("sql: " + sql);
...
var deleteChild = function(id, callback) {
	var sql = "DELETE FROM ?? WHERE ?? = ?";
	var selects = [ 'guardian', 'childfk', id ];
	sql = mysql.format(sql, selects);
	console.log("delete child sql");
	console.log(sql);
	runQuery(sql, function(err, result) {
		if (err) {
			callback(err);
 ...
 *            Name of the vaccine.
 * @param callback
 *            (err, res)=> res = newly created vaccine object.
 */
 ...
 * 
 * @param callback
 *            (err, res)=> res = list of all dose object.
 */
var getAllDoses = function(callback) {
	var sql = "SELECT dose.id, dose.vaccine, vaccine.title, dose.name, dose.dab, dose.period "
 ...
 *            Name of the vaccine.
 * @param dab
 *            when to apply the vaccine in days after birth.
 * @param period
 *            valid time range to apply.
 * @param callback
 *            (err, res)=> res = newly created vaccine object.
 */
var createDose = function(vaccine, name, dab, period, callback) {
	getDose(dab, name, vaccine, function(err, data) {
		if (data && data.length > 0) {
			...
			var sql = "INSERT INTO ?? (??, ??, ??, ??) VALUES(?, ?, ?, ?)";
			var inserts = [ 'dose', 'dab', 'period', 'name', 'vaccine', dab,
					period, name, vaccine ];
			sql = mysql.format(sql, inserts);
			runQuery(sql, callback);
		}
...
var setTaken = function(taken, callback) {
	...
	runQuery(mysql.format(sql, inserts), function(err, data) {
		...
		if (data && data.length > 0 && taken.check) { // exists but not
			// checked
 ...
 *            ID of an user
 * @param callback
 *            (err, res) => err is null if no error
 */
...
module.exports.createChildWithParent = createChildWithParent;
